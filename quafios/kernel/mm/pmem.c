...
void *ppalloc() {
    ...
    /* no enough memory? */
    if (!pfreelist.count) {
        panic(NULL, "Physical memory not enough!\n");
    }
...
void pmem_init() {
    ...
    for (i = 0; i < bootinfo->mem_ents; i++) {
        ...
        for (frame=base/PAGE_SIZE; frame<end/PAGE_SIZE; frame++) {
            if (frame < MEMORY_PAGES && pmmap[frame] != 0xFFFFFFFF) {
                /* a free page frame! */
